...
# Pragmatic's public catalogue occasionally exposes page tokens that happen to
# start with a provider-looking prefix (for example a mixed-case ``cs...`` token).
# Never trust those tokens merely because a regex matched them.  Known exceptional
# catalogue hints are only candidates: they still have to pass a real demo
# bootstrap before they are accepted.
...
def _looks_like_normal_symbol(value: str) -> bool:
    """Reject high-entropy page tokens while retaining normal Pragmatic IDs."""
...
def collect_symbol_candidates(text: str, final_url: str) -> list[tuple[str, str | None, str]]:
    """Return ordered symbol candidates without treating a regex hit as truth.

    Each tuple is ``(symbol, cver, evidence_url)``.  Launch URLs are strongest,
    then validated catalogue hints, then explicit page fields, then conservative
    lower-case fallback tokens.
    """
    ...
    # A real provider launch URL is the strongest evidence and is allowed to keep
    # its exact casing; bootstrap validation below is still authoritative.
    ...
    def _resolve_symbol_http(self, source_url: str, timeout_s: float) -> tuple[str, str | None, str]:
        ...
        for symbol, cver, evidence_url in candidates:
            try:
                ...
                continue
            try:
                # A candidate is accepted only after openGame/doInit calibration
                # actually resolves a usable session. This prevents random page
                # tokens from ever becoming persisted provider IDs.

...
_PRIVATE = re.compile(r"token|cookie|csrf|authorization|password|secret|session|t_key|launch_url", re.I)
...
def sanitize(value: Any, path: str = "") -> Any:
    ...
    if isinstance(value, dict):
        return {str(k): sanitize(v, f"{path}.{k}".strip(".")) for k, v in value.items()}

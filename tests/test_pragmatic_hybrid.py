...
class PragmaticHybridTests(unittest.TestCase):
    def test_catalog_uses_har_grounded_visible_dom_implementation(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            provider = PragmaticProvider(Path(temp))
            stop_event = threading.Event()
            logs: list[str] = []
            with patch(
                "tester_spin.providers.pragmatic_hybrid.crawl_pragmatic_catalog_dom",
                return_value=[],
            ) as crawl:
        ...
        self.assertTrue(any("tarjetas visibles" in line for line in logs))

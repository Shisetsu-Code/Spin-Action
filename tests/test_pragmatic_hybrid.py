...
class PragmaticHybridTests(unittest.TestCase):
    def test_catalog_uses_preloaded_dom_implementation(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            provider = PragmaticProvider(Path(temp))
            stop_event = threading.Event()
            logs: list[str] = []
            with patch(
                "tester_spin.providers.pragmatic_hybrid.crawl_pragmatic_catalog_preloaded",
                return_value=[],
            ) as crawl:
        ...
        self.assertTrue(any("DOM oculto/preloaded" in line for line in logs))

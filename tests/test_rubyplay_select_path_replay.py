...
class RubyPlaySelectPathReplayTests(unittest.TestCase):
    ...
    def test_untrusted_finite_domain_does_not_override_observed_downstream_index(self) -> None:
        runtime = SimpleNamespace(
            session=_Session(),
            default_bet=10,
            next_action="spin",
        ...
        select_indices: list[int] = []
        sequence = iter(["select", "select", "spin"])
        ...
        def post_action(rt, action, **kwargs):
            if action == "select":
                selected = int(kwargs["action_index"])
                select_indices.append(selected)
                if len(select_indices) == 2 and selected != 1:
                    raise ValueError("must prefer observed index over untrusted finite metadata")
            rt.next_action = next(sequence)
            return (
                SimpleNamespace(status_code=200),
                {"action": action, "index": kwargs.get("action_index")},
        ...
        result = GameTestResult(
            ...
            discovered_modes=[
                ...
                {
                    "id": "PURCHASE_SELECT__SELECT_INDEX_DOMAIN__PREFIX_SYNTHETIC",
                    "kind": "INDEXED_CHOICE",
        ...
        self.assertEqual(outcome["outcome"], "TERMINAL")
        self.assertEqual(select_indices, [2, 1])

    def test_downstream_select_uses_observed_index_instead_of_blind_zero(self) -> None:
        runtime = SimpleNamespace(
            session=_Session(),
            default_bet=10,
            next_action="spin",
        ...
        def post_action(rt, action, **kwargs):
            if action == "select":
                ...
                select_indices.append(selected)
                if len(select_indices) == 2 and selected != 1:
                    raise ValueError("downstream index must use observed legal option")
            rt.next_action = next(sequence)
            return (
                SimpleNamespace(status_code=200),
                {"action": action, "index": kwargs.get("action_index")},
